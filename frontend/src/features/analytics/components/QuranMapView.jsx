...
export default function QuranMapView() {
    ...
    const dataMap = useMemo(() => {
        const map = {};
        data.forEach(d => {
            const start = JUZZ_START_PAGES[d.juzz - 1];
            const localPage = d.page - start + 1;
            map[`${d.juzz}-${localPage}`] = d;
        });
        return map;
    }, [data]);

    const getActualPage = (j, p) =>
        (JUZZ_START_PAGES[j - 1] || 0) + (p - 1);
        ...
        <div className="chart-card quran-map-card">
            ...
            <div className="quran-map-screen-header">
                <div>
                    ...
                    <p className="heatmap-subtitle">Hover over cells to see page number, score, and comments.</p>
                </div>
            ...
            <div className="heatmap-scroll-container" ref={printRef}>
                <table className="heatmap-table">
                    <thead>
                        <tr>
                            <th className="juzz-header">Juzz / Pg</th>
                    ...
                    <tbody>
                        {Array.from({ length: 30 }, (_, i) => {
                            ...
                            return (
                                <tr key={j}>
                                    <td className="juzz-label">Juzz {j}</td>
