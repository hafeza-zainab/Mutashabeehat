// C:\quran-similarity-app\backend\scripts\import-quran.js
...
const ayahs = JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
console.log(`📊 Found ${ayahs.length} ayahs to import...`);

const sql = `INSERT OR REPLACE INTO ayahs (surah, ayah, page, text, juz, marhala, name) VALUES (?, ?, ?, ?, ?, ?, ?)`;
let insertedCount = 0;

db.serialize(() => {
    db.run('BEGIN TRANSACTION');
    ayahs.forEach((item) => {
        db.run(sql, [item.Surah, item.Ayah, item.Page, item.Text, item.Juz ?? item.Juzz, item.Marhala, SURAH_NAMES[item.Surah]], function(err) {
            if (err) console.error(`❌ Error inserting ${item.Surah}:${item.Ayah}`, err.message);
            else if (this.changes > 0) insertedCount++;
        });
    });
    db.run('COMMIT', (err) => {
        if (err) console.error('❌ Error committing:', err.message);
        else console.log(`✅ Successfully imported ${insertedCount} ayahs!`);
        db.close();
    });
});
